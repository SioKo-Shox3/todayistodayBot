...
| 決着(押下) | (b) `WithSession` が先に外し、入口は「もう居ない」と答える | 同左 | — |
| 決着(配札) | — | (b) **今回変更** 払ってから閉じる。失敗なら盤面が残り掃除人が払う | — |
| 時間切れ | 掃除人の `Remove`(入口を通らない)。精算が載って初めて外す | 同左 | 同左(`SettleTimedOutBoard`) |
...
- **期限切れの盤面は入口が触らない**。掃除人のものなので `Get` が false を返す時点で何もしない
  (`Close` 自身も期限切れを拒否するので、判定中に掃除人に取られても二重にはならない)。
