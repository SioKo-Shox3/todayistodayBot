...
## Next
- **C-3b は完了**(2026-09-15: C3B-01〜21 + P1〜P5 + X1・X2。Astra 1 周目 blocking 5 → 2 周目 blocking 1 → 以降はループ内評価者で収束)。これでフェーズ C(カジノ)が閉じた。`main` へ ff 済み。
- 次の候補: 稼働(トークンと実行場所はユーザー判断)、または新しいフェーズの M1。
...
## Notes
- **「預かりが取り残される」欠陥は 6 回続けて出た**(開始の順序 → 未送達の後始末 → 印の紐付け → 未送達 BJ → 配札時決着 BJ → 押下決着)。名指しした経路を塞ぐ直し方では終わらず、**機構を減らして**閉じた: 盤面を消せるのは `closeBoardIfSettled` の 1 か所だけ(`WithSession` から `done` による削除を撤去)、そこは「預かりの印がこの盤面を指している間は閉じない」しか見ない。新しい経路が増えても同じ穴が開かないことを `go/ast` の走査テストが守る。
- **ループランナーを二重に起動しない**(2026-09-15 に実際にやった)。同じ作業ツリーで 2 本走ると互いの編集を踏む。起動前に `Get-CimInstance Win32_Process` で `loop.mjs` が生きていないことを確かめる。反復側はこれを検知して 1 バイトも書かずに BLOCKED を出した(正しい挙動)。
