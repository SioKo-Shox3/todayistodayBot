...
## C3B-P1: 「預かりが無い」と「預かりが別のゲームのもの」を区別し、後者では盤面を落とさない(C3B-18 の差し戻し)
- status: done
- done-when: `NEXT_FINDINGS.md` の C3B-18 差し戻し 2 件を閉じる。(1) duel の返金(`store.go` 1790 付近)は、預かりが**別のゲームのもの**でも `ErrNoGameInProgress` を返すため、掃除人(`casino_sessions.go` 106 付近)がそれを「精算するものが無い」と解釈して盤面を削除する — 預かりは残ったままになる(再現: 同じ利用者に H&L の預かり 100 と別の印を置き、duel の期限切れ盤面を巡回させる)。**「預かりがそもそも無い(削除してよい)」と「預かりが別の盤面・別のゲームのもの(削除してはいけない)」を別のセンチネルで区別**し、後者は C3B-18 と同じく盤面を保持して警告を出し次の巡回で再試行する。H&L・BJ・duel の 3 経路すべてで同じ区別にする。(2) 「不一致のときに警告が出る」ことを**ログを捕まえて**検査する回帰テストを足す(既存テストは盤面保持・預かり・再試行までしか見ていない)。`slog` の出力を差し替えて検査する形は既存テストの流儀に合わせる。
- verify: `go build ./... && go vet ./...`
- verify: `go test ./internal/casino/... -count=1`
- verify: `go test ./internal/commands/... -count=1`
- verify: `go test ./... -count=1`
...
- verify: `go test ./internal/commands/... -count=1`
- verify: `go test ./... -count=1`
- paths: internal/commands/casino_shared.go, internal/commands/casino_shared_test.go, internal/commands/highlow.go, internal/commands/blackjack.go, internal/commands/duel.go, internal/commands/highlow_test.go, internal/commands/blackjack_test.go, internal/commands/duel_test.go
- notes: 文言だけで、資金の経路は変えない。
