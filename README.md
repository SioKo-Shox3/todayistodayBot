...
### カジノ（コイン・チップの経済）
- `/balance` - 自分のコイン・チップ・ストリーク・総資産を表示（本人のみに表示）
- `/daily` - デイリーボーナスを受け取る（1日1回、連続受け取りでボーナス増加）
- `/rate` - 今日のコイン⇔チップ両替レートと直近7日間のスパークラインを表示
- `/exchange coin-to-chip <amount>` / `/exchange chip-to-coin <amount>` - コインとチップを両替（手数料3%）
  - `amount`: 両替する枚数（1以上）
- `/slot <bet>` - スロットを回す
  - `bet`: ベット額（10〜1,000チップ）
  - ベット額の2%がサーバーごとのジャックポットプールに積み立てられます（配当表は変わらず、控除はベット額のままです）
  - 7️⃣7️⃣7️⃣ が揃うと通常配当に加えてプール全額を獲得し、プールは1,000チップに戻ります（💎💎💎 ではプールからの払い出しはありませんが、積立は他のスピンと同じように行われます）
  - 現在のプールは `/slot` の結果と毎朝9時の掲示に「🎰 ジャックポット: N チップ」として表示されます
- `/lottery buy <count>` - 宝くじを買う（1枚50チップ、毎日9時JSTに抽選）
  - `count`: 購入する枚数（1〜10）。1人あたり1回の抽選で10枚までです
  - 売上の90%（＋繰り越し）が当選者1人に支払われます。当選確率は購入枚数で重み付けされます
  - 残りの10%はジャックポットのプールに入ります
- `/lottery status` - 次回の賞金・売れた枚数・自分の枚数・次回の抽選時刻・前回の結果を表示
  - 購入者がいなかった日の賞金は次回へ繰り越されます。抽選結果は9時の掲示にも出ます
- `/rank` - 総資産（コイン×当日レート＋チップ＋預かり中のチップ）ランキングTOP10を表示
  - 預かり中のチップは、ゲーム中のベットとして口座から離れている分です（精算まで総資産に含めて数えます）
- `/season` - 今月のシーズン順位（純利）TOP10・自分の順位・今日を含む残り日数・前シーズンの結果を表示
  - シーズンは月ごとの区切りで、JSTの月初0時に切り替わります（その時刻にBotが落ちていても、次に動いたときの最初のコマンドで切り替わります）
  - 順位を決めるのはゲームの純利（受け取った額 − 賭けた額の累計）です。スロット・ハイ&ロー・ブラックジャック・`/duel`・宝くじが対象で、デイリーボーナス・両替・`/casino-admin mint`・シーズン賞与は含みません
  - 切り替えのときに上位3名へ賞与（1位10,000／2位5,000／3位2,500チップ）が支払われます。**残高はリセットされません** — 0に戻るのは純利の集計だけです
  - 今月の純利は `/balance` にも表示されます。保持される過去のシーズンは直近の1件です
- `/casino-admin mint <user> <amount>` - 指定ユーザーにコインを発行（サーバー管理者のみ）
  - `user`: コインを発行する相手 / `amount`: 発行するコインの枚数（1以上）
- `/casino-admin channel <channel>` - 毎朝9時のレート掲示先チャンネルを設定（サーバー管理者のみ）
  - `channel`: 掲示先のテキストチャンネル
- カジノのコマンドはサーバー内でのみ使えます（DMでは利用できません）

### カジノ（ボタン操作のゲーム）
- `/highlow <bet>` - ハイ&ローで遊ぶ
  - `bet`: ベット額（10〜1,000チップ）
  - 次のカードが今の1枚より高いか低いかを当て、当たるたびにポットが増えます。引いたカードは山に戻りません
  - ⬆️ハイ / ⬇️ロー / 💰キャッシュアウト のボタンは、そのゲームを開始した本人だけが押せます
- `/blackjack <bet>` - ブラックジャックで遊ぶ
  - `bet`: ベット額（10〜1,000チップ）
  - 🃏ヒット / ✋スタンド / ⏫ダブル をボタンで選びます。ディーラーは17以上でスタンド、ブラックジャックの配当は3:2
- どちらも3分間操作がないと自動で決着し、チップが精算されます
- `/duel <opponent> <bet>` - コイントスで1対1の勝負を挑む
  - `opponent`: 挑戦するユーザー（自分自身とBotは選べません） / `bet`: ベット額（10〜1,000チップ）
  - ⚔️受ける / 🚫断る のボタンを押せるのは挑戦された本人だけです。挑戦した時点でベット額が預かりに移り、相手が受けると相手からも同額を預かります
  - 勝者は自分のベットと相手のベットを受け取ります。コイントスなのでハウスの取り分はありません
  - 受け取れるのはチップの所持上限（1兆チップ）に収まる分までです。上限を超える分は支払われないため、上限近くで勝つと受取額はベット額の2倍に届きません
  - 断られたとき、および3分間受諾がないまま失効したときは、挑戦者のベットが全額返ります
